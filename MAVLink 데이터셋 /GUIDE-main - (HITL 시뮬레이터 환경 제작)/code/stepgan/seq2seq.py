...
class EncoderRNN(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, label_size: int, max_length: int):
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, label_size: int, output_size: int,
        ...
        self.attn = nn.Linear(self.hidden_size * 2, 32)
...
class Seq2Seq(nn.Module):
    ...
    @classmethod
    def build_model(cls, n_vocabs: int, decoder_output_size: int, hidden_size: int, label_size: int,
                    encoder_seq_len: int, decoder_seq_len: int, device: torch.device):
        encoder = EncoderRNN(n_vocabs, hidden_size, label_size, encoder_seq_len).to(device)
        decoder = AttnDecoderRNN(n_vocabs, hidden_size, label_size, decoder_output_size,
                                 0.25, decoder_seq_len).to(device)
    ...
    def forward(self, input_batch: torch.Tensor, label_batch: torch.Tensor, target_batch: torch.Tensor = None):
        ...
        encoder_outputs = torch.zeros(self.encoder.max_length, batch_size, self.encoder.hidden_size, device=self.device)
        encoder_hidden = self.encoder.init_hidden(batch_size, self.device)

        for seq_elem_idx in range(self.encoder.max_length):
        ...
        for seq_elem_idx in range(self.decoder.max_length):
            ...
            if seq_elem_idx >= self.decoder.max_length - 1:  # Index range 에러를 막기 위해 강제로 break
                break
